...
ike<-subset(ike, subset=!is.na(Hate.Incident)) #Ike already has an indicent number variable
...
Date<-paste(as.character(newdata_all2$day),as.character(newdata_all2$month),as.character(newdata_all2$year),sep="/")
newdata_all2$date<-as.Date(Date, "%d/%m/%Y")
newdata_all2<-newdata_all2[order(newdata_all2$date,newdata_all2$city),]
...
inc1<-cbind.data.frame(inc.id=7681.1,
                       ...
                       numperp=1,
                       intensity= 2,
                       notes="Additional incident found in article, Jewish Academy")
...
inc2<-cbind.data.frame(inc.id=7681.2,
                       ...
                       numperp=1,
                       intensity= 2,
                       notes="Additional incident found in article, Chabad South again")

inc2<-cbind.data.frame(inc.id=7681.3,
                       Title="Man indicted for hate crime and threats to Jewish Community Center and cyberstalking.1",
                       URL="http://www.wrbl.com/news/crime-watch/man-indicted-for-hate-crime-and-threats-to-jewish-community-center-and-cyberstalking/999050135",
                       day=5,
                       month=1,
                       ...
                       numperp=1,
                       intensity= 2,
                       notes="Additional incident found in article, Tampa JCC Preschool")
...
Date<-paste(as.character(newdata$day),as.character(newdata$month),as.character(newdata$year),sep="/")
newdata$date<-as.Date(Date, "%d/%m/%Y")
newdata<-newdata[order(newdata$date,newdata$city),]
